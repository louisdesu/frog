...
from .load_files import getInputLoadFile, get_user_input, loadData_shimmer, butter_lowpass_filter
...
def classify_noninteractive(file_path,
                            output_path,
                            classifier='Binary'):
    '''
    Inputs:
      classifier: a string, 'Binary', 'Multiclass', or 'Both'
    '''
    # Constants
    oneHour = 8*60*60 # 8(samp/s)*60(s/min)*60(min/hour) = samp/hour
    fiveSec = 8*5
    ...
    # Load data
    # TODO: This could be changed to load different kinds of data
    #   based on a file_type argument
    print("loading data")
    data = load_frog_file(file_path)

    # Get featureNames list
    valid_classifiers = ('Binary', 'Multiclass', 'Both')
    if classifier not in valid_classifiers:
        raise ValueError("classifier should be one of:" + str(valid_classifiers))
    if classifier == 'Both':
        classifiers = ['Binary', 'Multiclass']
    else: 
        classifiers = [classifier]
    feature_names = [getSVMFeatures(cls) for cls in classifiers]
    ...
    # Initialize labels array
    labels = -1 * np.ones((num_labels, len(classifiers)))
    ...
    for h in range(hours):
        ...
        for i in range(len(classifiers)):
            ...
            current_features = feature_names[i]
            # Label each 5 second epoch
            temp_labels = classifyEpochs(features, current_features, current_classifier)
            labels[(h*12*60):(h*12*60+temp_labels.shape[0]),i] = temp_labels
    ...
    featureLabels.rename(columns={'Binary': 'BinaryLabels', 
                                  'Multiclass': 'MulticlassLabels'},
                         inplace=True)

    featureLabels.to_csv(output_path)
